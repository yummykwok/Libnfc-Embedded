...
pn53x_transceive(struct nfc_device *pnd, const uint8_t *pbtTx, const size_t szTx, uint8_t *pbtRx, const size_t szRxLen, int timeout)
{
  ...
  PNCMD_TRACE(pbtTx[0]);
  if (timeout > 0) {
    /*log_put*/ printf("[Debug][pn53x] Timeout value: %d\r\n", timeout);
  } else if (timeout == 0) {
    /*log_put*/ printf("[Debug][pn53x] %s", "No timeout\r\n");
  } else if (timeout == -1) {
    timeout = CHIP_DATA(pnd)->timeout_command;
  } else {
    /*log_put*/ printf("[Error][pn53x] Invalid timeout value: %d\r\n", timeout);
  }

  static uint8_t  abtRx[PN53x_EXTENDED_FRAME__DATA_MAX_LEN];
...
//      if (pbtRx[0] & 0x40) { abort(); } // MI detected
  ...
  if (res < 0) {
    pnd->last_error = res;
    /*log_put*/ printf("[Debug][pn53x] Chip error: \"%s\" (%02x), returned error: \"%s\" (%d))\r\n", pn53x_strerror(pnd), CHIP_DATA(pnd)->last_status_byte, nfc_strerror(pnd), res);
  } else {
    pnd->last_error = 0;
  }
  return res;
                                 ...
                                 const size_t szRx, int timeout)
{
  ...
  if (pbtRx != NULL) {
    if (szRxLen >  szRx) {
      /*log_put*/ printf("[Error][pn53x] Buffer size is too short: %" PRIuPTR " available(s), %" PRIuPTR " needed\r\n", szRx, szRxLen);
      return NFC_EOVFLOW;
    }
    // Copy the received bytes
...
pn53x_initiator_transceive_bytes_timed(struct nfc_device *pnd, const uint8_t *pbtTx, const size_t szTx, uint8_t *pbtRx, const size_t szRx, uint32_t *cycles)
{
  ...
  if (pnd->bCrc) {
    ...
    if ((txmode & SYMBOL_TX_FRAMING) == 0x00)
      ...
      /*log_put*/ printf("[Error][pn53x] Unsupported framing type %02X, cannot adjust CRC cycles\r\n", txmode & SYMBOL_TX_FRAMING);
...
{
  int ret;
  /*log_put*/ printf("[Debug][pn53x] %s", "target_is_present(): Ping -4A\r\n");
  if (CHIP_DATA(pnd)->type == PN533) {
...
{
  int ret;
  /*log_put*/ printf("[Debug][pn53x] %s", "target_is_present(): Ping Jewel\r\n");
...
{
  int ret;
  /*log_put*/ printf("[Debug][pn53x] %s", "target_is_present(): Ping MFUL\r\n");
...
{
  int ret;
  /*log_put*/ printf("[Debug][pn53x] %s", "target_is_present(): Ping MFC\r\n");
...
{
  int ret;
  /*log_put*/ printf("[Debug][pn53x] %s", "target_is_present(): Ping DEP\r\n");
...
{
  /*log_put*/ printf("[Debug][pn53x] %s", "target_is_present(): Ping Felica\r\n");
...
{
  int ret;
  /*log_put*/ printf("[Debug][pn53x] %s", "target_is_present(): Ping -4B\r\n");
...
{
  int ret;
  /*log_put*/ printf("[Debug][pn53x] %s", "target_is_present(): Ping B'\r\n");
...
{
  int ret;
  /*log_put*/ printf("[Debug][pn53x] %s", "target_is_present(): Ping B2 ST SRx\r\n");
...
{
  int ret;
  /*log_put*/ printf("[Debug][pn53x] %s", "target_is_present(): Ping B2 ASK CTx\r\n");
...
pn53x_initiator_target_is_present(struct nfc_device *pnd, const nfc_target *pnt)
{
  ...
  if (CHIP_DATA(pnd)->current_target == NULL) {
    /*log_put*/ printf("[Debug][pn53x] %s", "target_is_present(): no saved target\r\n");
  ...
  if ((pnt != NULL) && (!pn53x_current_target_is(pnd, pnt))) {
    /*log_put*/ printf("[Debug][pn53x] %s", "target_is_present(): another target\r\n");
  ...
  switch (CHIP_DATA(pnd)->current_target->nm.nmt) {
    case NMT_ISO14443A:
      if (CHIP_DATA(pnd)->current_target->nti.nai.btSak & 0x20) {
        ...
        /*log_put*/ printf("[Debug][pn53x] %s", "target_is_present(): card type A not supported\r\n");
        ret = NFC_EDEVNOTSUPP;
      }
      break;
    case NMT_DEP:
    ...
    case NMT_ISO14443B2CT:
      ...
      break;
    default:
      /*log_put*/ printf("[Debug][pn53x] %s", "target_is_present(): card type not supported\r\n");
      ret = NFC_EDEVNOTSUPP;
      break;
  }
...
pn53x_check_ack_frame(struct nfc_device *pnd, const uint8_t *pbtRxFrame, const size_t szRxFrameLen)
{
  if (szRxFrameLen >= sizeof(pn53x_ack_frame)) {
    if (0 == memcmp(pbtRxFrame, pn53x_ack_frame, sizeof(pn53x_ack_frame))) {
      /*log_put*/ printf("[Debug][pn53x] %s", "PN53x ACKed\r\n");
      return NFC_SUCCESS;
    }
  }
  pnd->last_error = NFC_EIO;
  /*log_put*/ printf("[Error][pn53x] %s", "Unexpected PN53x reply!\r\n");
  return pnd->last_error;
}
...
pn53x_check_error_frame(struct nfc_device *pnd, const uint8_t *pbtRxFrame, const size_t szRxFrameLen)
{
  if (szRxFrameLen >= sizeof(pn53x_error_frame)) {
    if (0 == memcmp(pbtRxFrame, pn53x_error_frame, sizeof(pn53x_error_frame))) {
      /*log_put*/ printf("[Debug][pn53x] %s", "PN53x sent an error frame\r\n");
      pnd->last_error = NFC_EIO;
      return pnd->last_error;
    }
  }
...
pn53x_build_frame(uint8_t *pbtFrame, size_t *pszFrame, const uint8_t *pbtData, const size_t szData)
{
  if (szData <= PN53x_NORMAL_FRAME__DATA_MAX_LEN) {
    ...
    /*log_put*/ printf("[Error][pn53x] We can't send more than %d bytes in a raw (requested: %" PRIdPTR ")\r\n", PN53x_EXTENDED_FRAME__DATA_MAX_LEN, szData);
    return NFC_ECHIP;
  }
  return NFC_SUCCESS;
}
