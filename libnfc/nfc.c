...
/*Define driver for STM32*/
const struct nfc_driver *uart_driver = &pn532_uart_driver;
 ...
 * @param port The UART port specified to send/receive bytes, port must be initialized in advance
...
nfc_device *
nfc_open(const USART_TypeDef* port)
{
  nfc_device *pnd  = uart_driver->open((void*)port);
  if(pnd!=NULL){
    /*log_put*/ printf("[Debug][nfc] NFC device has been claimed.\r\n");
  }else{
    /*log_put*/ printf("[Error][nfc.internal] Failed to open NFC device\r\n");
  }
  return pnd;
}
 ...
 */
size_t
nfc_list_devices(const void* ports[], const size_t szPorts)
{
  /*This function is under implementation*/
  return 0;
}
                                    ...
                                    nfc_target *pnt)
{
  ...
  uint8_t *abtTmpInit=(uint8_t *)malloc(sizeof(uint8_t)*MAX(12, szInitData));
  size_t  szInit = 0;
  if (szInitData == 0) {
